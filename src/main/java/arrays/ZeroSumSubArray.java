...
Now if at any point prefix_sum==0 , ie subarry from (0) to (y) is zero => that's why we set first_occ[0]=-1;
(coz starting se y point tk is zero array and length of that array will be = y - (-1). here y represents the index at point where prefixsum becomes zero.

1. we first take one hashmap 'first_occ' which will store the first occurance of the prefix sum
2. set first_occ[0]=-1
3. Iterate over the array 3.1) calculate the prefix_sum. 3.2) check if our hashmap 'first_occ' contains
prefix_sum or not 3.2.1) If prefix_sum doesnot exist -> set it's first occurance as 'i'. (first_occ[prefix_sum]=i) 3.3) Now calculate the size of zero subarray . int size = i - first_occ[arr[i]]; 3.4) Now update the value of max_size (variable storing the maximum size of subarray having sum=0)
